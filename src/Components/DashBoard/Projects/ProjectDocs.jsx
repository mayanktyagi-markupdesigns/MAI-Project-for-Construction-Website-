...
const PENDING_ACTION_KEY = "pendingProjectDocsAction";
...
export default function ProjectDocs() {
  ...
  const openProjectDocsFolder = (project) => {
    ...
    const ProjectDocsFolder = project.subfolders.find(
      (sf) => sf.name === "Project Docs"
    );
    if (!ProjectDocsFolder) {
      Swal.fire({
        icon: "info",
        title: "No Project Docs Files folder",
        text: "Change Order folder not found in this project.",
      });
      return;
    }

    const url = `https://drive.google.com/drive/folders/${ProjectDocsFolder.drive_folder_id}`;
            ...
            {loading ? (
              ...
              filteredProjects.map((p) => (
                <tr key={p.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <button
                      onClick={() => openProjectDocsFolder(p)}
