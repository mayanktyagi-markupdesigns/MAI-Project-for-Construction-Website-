...
const PENDING_ACTION_KEY = "pendingPaymentAction";
...
export default function Payment() {
 ...
 const getLocalProjects = () => {
  ...
  const openPaymentFolder = (project) => {
    ...
    const PaymentFolder = project.subfolders.find(
      (sf) => sf.name === "Payment"
    );
    if (!PaymentFolder) {
      Swal.fire({
        icon: "info",
        title: "No Payment Files folder",
        text: "Change Order folder not found in this project.",
      });
      return;
    }

    const url = `https://drive.google.com/drive/folders/${PaymentFolder.drive_folder_id}`;
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-2 md:gap-0">
        <div className="text-center md:text-left">
          ...
          <p className="text-gray-600 bricolage-grotesque text-sm md:text-base mt-1">
            Scan specifications and manage Payment
          </p>
        </div>
