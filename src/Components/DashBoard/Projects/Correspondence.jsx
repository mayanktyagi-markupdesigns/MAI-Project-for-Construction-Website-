...
const PENDING_ACTION_KEY = "pendingCorrespondenceAction";
...
export default function Correspondence() {
 ...
 const getLocalProjects = () => {
  ...
  const openCorrespondenceFolder = (project) => {
    ...
    const CorrespondenceFolder = project.subfolders.find(
      (sf) => sf.name === "Correspondence"
    );
    ...
    const url = `https://drive.google.com/drive/folders/${CorrespondenceFolder.drive_folder_id}`;
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-2 md:gap-0">
        <div className="text-center md:text-left">
          ...
          <p className="text-gray-600 bricolage-grotesque text-sm md:text-base mt-1">
            Scan specifications and manage Correspondence
          </p>
        </div>
            ...
            {loading ? (
              ...
              filteredProjects.map((p) => (
                <tr key={p.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <button
                      onClick={() => openCorrespondenceFolder(p)}
