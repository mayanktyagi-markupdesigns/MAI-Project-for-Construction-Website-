...
const PENDING_ACTION_KEY = "pendingCloseoutAction";
...
export default function Closeout() {
  ...
  const openCloseoutFolder = (project) => {
    ...
    const CloseoutFolder = project.subfolders.find(
      (sf) => sf.name === "Closeout"
    );
    if (!CloseoutFolder) {
      Swal.fire({
    ...
    const url = `https://drive.google.com/drive/folders/${CloseoutFolder.drive_folder_id}`;
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-2 md:gap-0">
        <div className="text-center md:text-left">
          ...
          <p className="text-gray-600 bricolage-grotesque text-sm md:text-base mt-1">
            Scan specifications and manage Closeout
          </p>
        </div>
