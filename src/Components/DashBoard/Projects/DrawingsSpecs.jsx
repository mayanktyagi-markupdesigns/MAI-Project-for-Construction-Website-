...
const PENDING_ACTION_KEY = "pendingDrawingsSpecsAction";

export default function DrawingsSpecs() {
  ...
  const openDrawingsSpecsFolder = (project) => {
    ...
    const DrawingsSpecsFolder = project.subfolders.find(
      (sf) => sf.name === "Drawings Specs"
    );
    if (!DrawingsSpecsFolder) {
      Swal.fire({
        icon: "info",
        title: "No Daily Reports folder",
        text: "Change Order folder not found in this project.",
      });
      return;
    }

    const url = `https://drive.google.com/drive/folders/${DrawingsSpecsFolder.drive_folder_id}`;
    ...
    <div className="max-w-full mx-auto p-2">
            ...
            {loading ? (
              ...
              filteredProjects.map((p) => (
                <tr key={p.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <button
                      onClick={() => openDrawingsSpecsFolder(p)}
      ...
      {isUploadModalOpen && (
        <DocumentUploadModal
          ...
          defaultCategory="Drawings Specs"
        />
      )}
    </div>
  );
