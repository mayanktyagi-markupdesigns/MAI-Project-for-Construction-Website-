...
const PENDING_ACTION_KEY = "pendingMeetingsAction";
...
export default function Meetings() {
 ...
 const getLocalProjects = () => {
  ...
  const openMeetingsFolder = (project) => {
    ...
    const MeetingsFolder = project.subfolders.find(
      (sf) => sf.name === "Meetings"
    );
    if (!MeetingsFolder) {
      Swal.fire({
        icon: "info",
        title: "No Meetings Files folder",
        text: "Change Order folder not found in this project.",
      });
      return;
    }

    const url = `https://drive.google.com/drive/folders/${MeetingsFolder.drive_folder_id}`;
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-2 md:gap-0">
        <div className="text-center md:text-left">
          ...
          <p className="text-gray-600 bricolage-grotesque text-sm md:text-base mt-1">
            Scan specifications and manage Meetings
          </p>
        </div>
