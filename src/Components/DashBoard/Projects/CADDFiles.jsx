...
const PENDING_ACTION_KEY = "pendingCADDFilesAction";

export default function CADDFiles() {
  ...
  const openCADDFilesFolder = (project) => {
    ...
    const CADDFilesFolder = project.subfolders.find(
      (sf) => sf.name === "CADD Files"
    );
    if (!CADDFilesFolder) {
      Swal.fire({
        icon: "info",
        title: "No CADD Files folder",
        text: "CADDFiles folder not found in this project.",
      });
      return;
    }

    const url = `https://drive.google.com/drive/folders/${CADDFilesFolder.drive_folder_id}`;
