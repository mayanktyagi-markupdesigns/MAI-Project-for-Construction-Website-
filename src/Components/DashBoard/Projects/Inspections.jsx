...
const PENDING_ACTION_KEY = "pendingInspectionsAction";
...
export default function Inspections() {
 ...
 const getLocalProjects = () => {
  ...
  const openInspectionsFolder = (project) => {
    ...
    const InspectionsFolder = project.subfolders.find(
      (sf) => sf.name === "Inspections"
    );
    if (!InspectionsFolder) {
      Swal.fire({
        icon: "info",
        title: "No Inspections Files folder",
        text: "Change Order folder not found in this project.",
      });
      return;
    }

    const url = `https://drive.google.com/drive/folders/${InspectionsFolder.drive_folder_id}`;
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-2 md:gap-0">
        <div className="text-center md:text-left">
          ...
          <p className="text-gray-600 bricolage-grotesque text-sm md:text-base mt-1">
            Scan specifications and manage Inspections
          </p>
        </div>
            ...
            {loading ? (
              ...
              filteredProjects.map((p) => (
                <tr key={p.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <button
                      onClick={() => openInspectionsFolder(p)}
