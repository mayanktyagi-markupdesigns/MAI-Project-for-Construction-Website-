...
const PENDING_ACTION_KEY = "pendingDailyReportsAction";
...
export default function DailyReports() {
  ...
  const openDailyReportsFolder = (project) => {
    ...
    const DailyReportsFolder = project.subfolders.find(
      (sf) => sf.name === "Daily Reports"
    );
    if (!DailyReportsFolder) {
      Swal.fire({
        icon: "info",
        title: "No Daily Reports Files folder",
        text: "Change Order folder not found in this project.",
      });
      return;
    }

    const url = `https://drive.google.com/drive/folders/${DailyReportsFolder.drive_folder_id}`;
            ...
            {loading ? (
              ...
              filteredProjects.map((p) => (
                <tr key={p.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <button
                      onClick={() => openDailyReportsFolder(p)}
